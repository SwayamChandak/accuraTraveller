...
def get_info(url, print_html=True):
    ...
    # Print the full HTML content before parsing
    if print_html:
        print("\n" + "="*80)
        print(f"FULL HTML CONTENT FROM: {url}")
        print("="*80)
        print(webpage.text)
        print("="*80)
        print("\n")
        ...
        restaurant_slug = url.split('/')[-1]
        with open(f"scraped_html/zomato_info_{restaurant_slug}.html", "w", encoding="utf-8") as f:
            f.write(webpage.text)
        print(f"✓ Full HTML saved to: scraped_html/zomato_info_{restaurant_slug}.html\n")
...
def get_restaurant_info(url_list, save=True, file_name="Restaurants.csv", print_html=True):
    """ Get Restaurant Information from all urls passed """

    # Collecting the data
    data = []
    for idx, url in enumerate(url_list):
        # Only print HTML for the first URL to avoid too much output
        data.append(get_info(url, print_html=(print_html and idx == 0)))
